  ...
  function Node(config) {
    ...
    this.on('input', function(msg) {
      ...
      var actionComplete = function(err, data) {
        if (err || data.status === 'ERROR') {
          node.status({
            fill: 'red',
            shape: 'ring',
            text: 'call to speech to text service failed',
          });
          node.error(err, msg);
        } else {
          var r = data.results;
      ...
      function performAction(audio, format, cbdone, cbcleanup) {
        ...
        if (
          config.langcustom && 'NoCustomisationSetting' !== config.langcustom
        ) {
      ...
      if (!username || !password) {
        var message_err_credentials =
          'Missing Speech To Text service credentials';
      ...
      if (!config.lang) {
        var message_err_lang =
          'Missing audio language configuration, unable to process speech.';
      ...
      if (!config.band) {
        var message_err_band =
          'Missing audio quality configuration, unable to process speech.';
      ...
      if (!config.continuous) {
        var message_err_continuous =
          'Missing continuous details, unable to process speech.';
      ...
      if (!msg.payload instanceof Buffer || !typeof msg.payload === 'string') {
        message =
          'Invalid property: msg.payload, can only be a URL or a Buffer.';
        ...
        switch (f) {
          ...
          default:
            var message_err_format =
              'Audio format (' +
              f +
              ') not supported, must be encoded as WAV, FLAC or OGG.';
      ...
      if (msg.payload instanceof Buffer) {
        temp.open({ suffix: '.' + fileType(msg.payload).ext }, function(
          err,
          info
        ) {
          if (err) {
            node.status({
              fill: 'red',
              shape: 'ring',
              text: 'unable to open audio stream',
            });
        ...
        message =
          'Payload must be either an audio buffer or a string representing a url';
        node.error(message, msg);
        return;
      }
    });
