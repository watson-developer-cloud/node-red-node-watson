...
module.exports = function(RED) {
	...
	function Node(config) {
		...
		var node = this;
			...
			this.on('input', function(msg) {
				...
				var apikey = sApikey || this.credentials.apikey;
				//username = username || this.credentials.username;
				//password = password || this.credentials.password;

				//if (!username || !password) {
				if (!apikey) {
					var message = 'Missing Visual Recognition service v3 credentials (api_key)';
					node.error(message, msg)
					return;
				}
	...
	RED.nodes.registerType("visual-recognition-v3",Node, {
		credentials: {
	...
	RED.nodes.registerType("visual-util-v3",VisualUtilNode);
	...
	RED.nodes.registerType("visual-training-v3",VisualTrainingNode);
};
